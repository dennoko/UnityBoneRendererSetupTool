...
namespace Hays.BoneRendererSetup.UI
{
    ...
    ///   TextCore がグリフ生成に失敗し文字が一切表示されなくなるため使用禁止。
    /// </summary>
    [InitializeOnLoad]
    internal static class DennokoUIFont
    {
        ...
        // ─── ウォームアップ文字 ─────────────────────────────────────────────
        // 生成時にまとめてアトラスへ焼き込み、実行中のアトラス追加（＝保護漏れの隙）を減らす。
        // UI に文言を追加したらここにも文字を足す。
        ...
        private const string WarmupJapanese =
            "→、。あいかがぎくこさしじすずせただつてでとなにねのはべまみむめもらりるれをん" +
            "アィイウエカキクグケシジスセタダチッツテトドノバパヒプペボマミャュユラリルン・ー" +
            "一上下他位体使保個値側先公内出則削前効動化参反右同名命問回在均変太失子定対左式" +
            "必手択持探揃敗数映時更有期果検機歪済無照現用目確索結編置肩背胸能腕腿自衣装要見" +
            "規設認象足転軸選配鎖開防除際集順頭題顎首骨（）";
        ...
        /// CreateGUI() / CreateInspectorGUI() から 1 回呼ぶ。
        /// </summary>
        ...
        private static FontAsset Get()
        {
            ...
            if (_font == null)
            {
                _unavailable = true;   // フォント未搭載環境。毎ティック再試行しない
                return null;
            }
            return _font;
        }
        ...
        /// 冪等。実行中に追加されたアトラスを拾うため定期的に呼び直すこと。
        /// </summary>
        ...
        // ─── 監視ティック ───────────────────────────────────────────────────
        // 動的に生成される文字列（ボーン名・オブジェクト名・エラーメッセージ等）でもアトラスは
        // 増える。ウィンドウが開いている間だけ低頻度で Protect を回して保護漏れを塞ぐ。
